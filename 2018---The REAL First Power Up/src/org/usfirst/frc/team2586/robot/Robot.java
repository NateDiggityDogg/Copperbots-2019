...
public class Robot extends TimedRobot {
	...
	private void autoProgSwitchCenter() {
		...
		case 1:
			if (autoDrive(24.0, 0.0))
				autoNextStep();
			break;

		case 2:
			if (autoDrive(0.0, rot))
				autoNextStep();
			break;

		case 3:
			if (autoDrive(48.0, -rot))
				autoNextStep();
			break;

		case 4:
			if (autoDrive(36.0, 0.0))
				autoNextStep();
			break;
	...
	private boolean autoDrive(double distance, double angle) {
		...
		/** Is this max acceleration speed? */
		...
		if (lin > 0)
			lin = Math.min(lin, maxSpeed);
		else
			lin = Math.max(lin, -maxSpeed);
		...
		// Max rotation speed
		if (rot > 0)
			rot = Math.min(lin, maxSpeed);
		else
			rot = Math.max(lin, -maxSpeed);
